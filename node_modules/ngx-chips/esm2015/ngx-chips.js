...
const KeyboardEvent = (/** @type {?} */ (global)).KeyboardEvent;
const MouseEvent = (/** @type {?} */ (global)).MouseEvent;
// mocking navigator
...
// angular universal hacks
/* tslint:disable-next-line */
const DragEvent = (/** @type {?} */ (global)).DragEvent;
...
class TagInputComponent extends TagInputAccessor {
    ...
    constructor(renderer, dragProvider) {
        ...
        this.onPasteCallback = (data) => {
            const /** @type {?} */ text = data.clipboardData.getData('text/plain');
    ...
    addItem(fromAutocomplete = false, item, index) {
         ...
         * @param tag
         */
        const /** @type {?} */ validationFilter = (tag) => {
            const /** @type {?} */ isValid = this.isTagValid(tag, fromAutocomplete) && this.inputForm.form.valid;
            if (!isValid) {
        ...
        of$1(model).pipe(first(), filter(() => model.trim() !== ''), map(() => item), map(this.createTag), filter(validationFilter)).subscribe(subscribeFn, undefined, reset);
    }
    /**
    ...
    setUpTextChangeSubscriber() {
        this.inputForm.form
            .valueChanges
            .pipe(debounceTime(this.onTextChangeDebounce), map(() => this.formValue))
